...
add_shortcode('monthly_breakdown', function() { 
    ...
    for ($m = 1; $m <= 12; $m++) { 
        ...
        foreach ($tenants as $tenant) {
            ...
            if (empty($t_start) || $t_start === '0000-00-00') continue;

            $month_start_ts = strtotime($first_day_of_month);
            $month_end_ts   = strtotime($last_day_of_month);
            $t_start_ts     = strtotime($t_start);
            $t_end_ts       = (!empty($t_end) && $t_end !== '0000-00-00') ? strtotime($t_end) : 0;

            // If strtotime fails, it returns false. We should ignore these.
            if ($month_start_ts === false || $month_end_ts === false || $t_start_ts === false) continue;
            ...
            $is_active = true;
            if ($t_start_ts > $month_end_ts) $is_active = false;
            if ($is_active && $t_end_ts !== 0 && $t_end_ts < $month_start_ts) $is_active = false;
