...
add_shortcode('ef_log_payment_btn', function() {   
    ...
    function efSubmitNewPaymentRecord() {   
        ...
        if(!tenant || !amount || !date) {   
            alert('Please select a tenant and enter the payment parameters.');   
            return;   
        }   
...
function ef_callback_ajax_create_payment() {   
    ...
    if($new_payment_id && !is_wp_error($new_payment_id)) {   
        update_field('associated_tenant', $tenant_id, $new_payment_id);    
        update_field('amount_paid', $amount, $new_payment_id);             
        update_field('date_of_payment', $date, $new_payment_id);   
        update_field('mode_of_payment', $method, $new_payment_id);   
   
        wp_send_json_success(['payment_id' => $new_payment_id]);   
    }   
...
function ef_callback_ajax_update_payment() { 
    ...
    if (!$payment_id || !$tenant_id || !$amount || !$date) {   
        wp_send_json_error(['message' => 'Operational parameters data validation failure.']);   
    }   
    ...
    if($updated_payment_id && !is_wp_error($updated_payment_id)) {   
        update_field('associated_tenant', $tenant_id, $payment_id);    
        update_field('amount_paid', $amount, $payment_id);             
        update_field('date_of_payment', $date, $payment_id);   
        update_field('mode_of_payment', $method, $payment_id);   
...
add_shortcode('ef_payments_stats', function() {   
    ...
    if ($month_query->have_posts()) {   
        while ($month_query->have_posts()) {   
            ...
            if ($amount) {   
                $this_month_revenue += floatval($amount);   
            }   
        }   
        wp_reset_postdata();   
    }   
    ...
    <div class="ef-stats-cards-row">   
        ...
        <div class="ef-stat-card-surface">   
            ...
            <div class="ef-stat-card-value ef-text-accent-green">   
                AED <?php echo number_format($this_month_revenue); ?>   
            </div>   
        </div>   
...
if ($query->have_posts()) {     
    while ($query->have_posts()) {     
        ...
        $tenant_id  = get_field('associated_tenant', $payment_id);     
        $amount     = get_field('amount_paid', $payment_id);     
        $raw_date   = get_field('date_of_payment', $payment_id);     
        $method     = get_field('mode_of_payment', $payment_id);     
        ...
        $property_name = $tenant_id ? get_field('property_unit_name', $tenant_id) : '';     
        if (!$property_name) {     
...
<style>     
    ...
    @media (max-width: 768px) {     
        .ef-ledger-control-bar { flex-direction: column; align-items: stretch; }     
        .ef-ledger-select-filter { max-width: 100%; }     
        .ef-ledger-metric-tally { margin-left: 0; text-align: left; padding-top: 6px; }     
        .ef-ledger-table-wrapper { overflow-x: auto; -webkit-overflow-scrolling: touch; }    
        .ef-core-tabular-matrix { min-width: 700px; }    
    }     
    ...
    .ef-btn-row-action.delete:hover { 
...
function efProcessLedgerQueryMatrix() {     
            ...
            rowHtml.innerHTML = `     
                ...
                <td style="text-align: right;">     
                    <button class="ef-btn-receipt-pdf-download" title="Download PDF" onclick='efDownloadReceipt(efLedgerSourcePayload.find(item => item.id === ${entry.id}))'>     
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="display:inline-block; vertical-align:middle; margin-top:-2px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>     
                    </button>     
    ...
    document.getElementById('efLedgerCounterDisplay').innerHTML = `${matchCount} results • <span>AED ${revenueAccumulator.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0})}</span>`;     
...
function efDownloadReceipt(d) {  
        ...
        <div class="receipt-container" style="padding:40px; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color:#334155; width:800px; background:#fff;">  
            <div class="receipt-header" style="display:flex; justify-content:space-between; border-bottom:2px solid #f1f5f9; padding-bottom:20px;">  
                ...
                <div style="text-align:right;">    
                    ...
                    <div style="color:#64748b; font-size:13px;">Receipt Reference: <span id="r-ref" style="font-weight:700; color:#0f172a;">${esc(ref)}</span></div>    
                </div>    
            </div>    
            <div style="display:flex; justify-content:space-between; margin:32px 0;">    
                <div>    
                    ...
                    <div style="font-size:20px; font-weight:700; color:#0f172a; margin-bottom:4px;">${esc(tenant)}</div>   
                    <div style="color:#64748b; font-size:14px;">${esc(prop)}</div>   
                </div>   
            ...
            <div style="display:flex; justify-content:flex-end; align-items:center; gap:32px; margin-top:32px;">   
                ...
                <span style="color:#00a86b; font-size:24px; font-weight:800; letter-spacing:-0.5px;">AED <span>${esc(amount)}</span></span>   
            </div>   
    ...
    const element = wrapper.querySelector(".receipt-container");  
