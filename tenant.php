...
$tenant_list_logic = function() {
    ...
    <style>
        ...
        .ef-tenant-card-item:hover, .ef-tenant-card-item.ef-active-tenant {
            border-color: #0a8a4f;
            background: #f0fdf4;
        }
        .ef-tenant-meta {
        ...
        .ef-tenant-rent-badge {
            font-weight: 700;
            color: #0a8a4f;
        }
        .ef-payment-status-badge {
            font-size: 10px;
            padding: 2px 8px;
            border-radius: 12px;
    ...
    <div class="ef-dashboard-sidebar-independent">
        ...
        <div id="tenant-dashboard-wrapper">
            <div id="tenant-dashboard-grid" class="tenant-dashboard-grid">
                <?php if ($q->have_posts()) : while ($q->have_posts()) : $q->the_post();
                    $tid=get_the_ID();
                    if (function_exists('get_field')) {
                        $rent = get_field('monthly_rent', $tid);
                        $pid  = get_field('property_id', $tid);
                    } else {
                        $rent = get_post_meta($tid, 'monthly_rent', true);
                        $pid  = get_post_meta($tid, 'property_id', true);
                    }
                    $name=get_the_title();
                    $addr = 'No Address';
                    if ($pid) {
                        if (function_exists('get_field')) {
                            $addr = get_field('add_address', $pid) ?: get_the_title($pid);
                        } else {
                            $addr = get_post_meta($pid, 'add_address', true) ?: get_the_title($pid);
                        }
                    }
                    ...
                    if($p_query->have_posts()){
                        while($p_query->have_posts()){
                            ...
                            $p_id = get_the_ID();
                            if (function_exists('get_field')) {
                                $amt_paid = get_field('amount_paid', $p_id);
                            } else {
                                $amt_paid = get_post_meta($p_id, 'amount_paid', true);
                            }
                            $paid_this_month += floatval($amt_paid);
                        }
                        wp_reset_postdata();
                    }
            ...
            <div class="tenant-dashboard-nav">
                <button id="tenant-prev-page" class="tenant-nav-btn" <?php echo ($paged <= 1) ? 'disabled' : ''; ?> onclick="window.location.href='<?php echo esc_url(add_query_arg('paged', $paged - 1)); ?>'">Previous</button>
                <span id="tenant-page-info">Page <?php echo $paged; ?> of <?php echo $total_pages; ?></span>
                <button id="tenant-next-page" class="tenant-nav-btn" <?php echo ($paged >= $total_pages) ? 'disabled' : ''; ?> onclick="window.location.href='<?php echo esc_url(add_query_arg('paged', $paged + 1)); ?>'">Next</button>
            </div>
        </div>
    </div>
    <script>
...
add_shortcode('tenant_profile', function() {
    ...
    <style>
        ...
        .tenant-nav-btn:hover:not(:disabled) {
            background: #f8fafc;
            border-color: #cbd5e1;
            color: #0f172a;
        }
        .tenant-nav-btn:disabled {
            opacity: 0.5;
            cursor: not-allowed;
        }
        #tenant-page-info {
    ...
    <div id="efTenantProfileModal" class="ef-popup-modal-container" style="display:none;" onclick="if(event.target === this) { this.style.display='none'; efToggleScrollLock(); }">
        <div class="ef-popup-modal-box" style="max-width: 800px; width: 90%; position: relative;">
            <input type="hidden" id="ef_tenant_profile_nonce" value="<?php echo wp_create_nonce('tenant_profile_nonce'); ?>">
    ...
    (function(){
        ...
        window.addEventListener('ef_load_tenant', e => {
            ...
            fd.append('tenant_id', e.detail.id);
            const nonceEl = document.getElementById('ef_tenant_profile_nonce');
            if (nonceEl) {
...
add_shortcode('tenant_manager', function(){
    ...
    '<div style="font-family:system-ui, sans-serif;">' .
           ...
           '<div class="ef-manager-flex-container" style="display:flex; gap:24px; flex-wrap:wrap; align-items:flex-start;">' .
...
if ( ! function_exists('eftm_get_total_units') ) {
    function eftm_get_total_units($property_id) {
        if (function_exists('get_field')) {
            ...
            $units = get_post_meta($property_id, 'number_unit', true);
...
function eftm_handle_check_property_availability() {
    ...
    if (function_exists('get_field')) {
        $property_address = get_field('add_address', $property_id) ?: get_the_title($property_id);
    } else {
        $property_address = get_post_meta($property_id, 'add_address', true) ?: get_the_title($property_id);
    }
...
function eftm_handle_ajax_tenant_breakdown() {
    ...
    if (function_exists('get_field')) {
        ...
        $monthly_rent   = floatval(get_post_meta($tenant_id, 'monthly_rent', true));
        $due_day        = get_post_meta($tenant_id, 'rent_due_day', true);
        $email          = get_post_meta($tenant_id, 'tenant_email', true);
        $phone          = get_post_meta($tenant_id, 'tenant_phone', true);
        $address        = get_post_meta($tenant_id, 'tenant_address', true);
        $contract_start = get_post_meta($tenant_id, 'tenant_start', true);
        $contract_end   = get_post_meta($tenant_id, 'tenant_end', true);
        $unit           = get_post_meta($tenant_id, 'tenant_unit', true);
        $prop_field     = get_post_meta($tenant_id, 'property_id', true);
    }
    ...
    if ($p_query->have_posts()) {
        while ($p_query->have_posts()) {
            ...
            if (function_exists('get_field')) {
                ...
                $method   = get_field('mode_of_payment', $pay_id);
                $amount   = get_field('amount_paid', $pay_id);
            } else {
                $raw_date = get_post_meta($pay_id, 'date_of_payment', true);
                $method   = get_post_meta($pay_id, 'mode_of_payment', true);
                $amount   = get_post_meta($pay_id, 'amount_paid', true);
            }
    ...
    <div class="ef-meta-grid-3col">
        <div class="ef-meta-info-card"><div class="ef-meta-card-label">Monthly rent</div><div class="ef-meta-card-value ef-text-green">AED <?php echo number_format($monthly_rent); ?></div></div>
        ...
        <?php if (!empty($payment_history)) : foreach ($payment_history as $p) : ?>
            <div class="ef-payment-row-card">
                ...
                <div class="ef-pay-row-right">
                    <div class="ef-pay-amount-value">AED <?php echo number_format($p['amount']); ?></div>
                    <button class="ef-btn-receipt"
                            ...
                            data-amount="<?php echo number_format($p['amount']); ?>">
...
function eftm_global_execute_properties_fetch() {
    ...
    if ($q->have_posts()) {
        while ($q->have_posts()) {
            ...
            $id = get_the_ID();
            if (function_exists('get_field')) {
                $p_addr     = get_field('add_address', $id);
                $p_rent     = get_field('property_rent', $id);
                $p_start    = get_field('property_start_date', $id);
                $p_end      = get_field('property_end_date', $id);
                $p_due      = get_field('property_due_day', $id);
            } else {
                $p_addr     = get_post_meta($id, 'add_address', true);
                $p_rent     = get_post_meta($id, 'property_rent', true);
                $p_start    = get_post_meta($id, 'property_start_date', true);
                $p_end      = get_post_meta($id, 'property_end_date', true);
                $p_due      = get_post_meta($id, 'property_due_day', true);
            }
            $res[] = [
